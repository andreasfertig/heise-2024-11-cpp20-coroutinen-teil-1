...
struct Chat {
  struct promise_type {
    std::string _msgOut{};
    std::string _msgIn{};  // #A Storing a value from
                           // or for the coroutine
    ...
    std::suspend_always yield_value(
      std::string
        msg) noexcept  // #F Value from co_yield
    {
    ...
    auto await_transform(
      std::string) noexcept  // #G Value from co_await
    {
      struct awaiter {  // #H Customized version
                        // instead of using
                        // suspend_always or
                        // suspend_never
        promise_type&  pt;
  ...
  {}  // #B Get the handle form the promise
  Chat(Chat&& rhs)
  : mHandle{std::exchange(rhs.mHandle, nullptr)}
  {}  // #C Move only!

  ~Chat()  // #D Care taking, destroying the handle if
           // needed
  {
    if(mHandle) { mHandle.destroy(); }
  }

  std::string listen()  // #E Active the coroutine and
                        // wait for data.
  {
  ...
  void answer(
    std::string msg)  // #F Send data to the coroutine
                      // and activate it.
  {
    mHandle.promise()._msgIn = msg;
    if(not mHandle.done()) { mHandle.resume(); }
  }
};

Chat Fun()  // #A Wrapper type Chat containing the
            // promise type
{
  co_yield "Hello!\n"s;  // #B Calls
                         // promise_type.yield_value

  std::cout << co_await std::
      string{};  // #C Calls
...
int main()
{
  ...
  std::cout
    << marco.listen();  // #F Trigger the machine

  marco.answer(
    "Where are you?\n"s);  // #G Send data into the
                           // coroutine

  std::cout
    << marco.listen();  // #H Wait for more data from
                        // the coroutine
}